...
class _EditTaskPageState extends State<EditTaskPage> {
  ...
  void initState() {
    ...
    _selectedCategory =
        _categories.contains(rawMateria) ? rawMateria : 'General';
    _selectedPriority = widget.initialData['priority'] ?? 'media';
    _selectedReminderLevel = NotificationService.reminderLevelFromValue(
        widget.initialData['reminderLevel']);

    if (widget.initialData['dueDate'] != null &&
        widget.initialData['dueDate'] is Timestamp) {
      final potentialDate =
          (widget.initialData['dueDate'] as Timestamp).toDate();
      if (potentialDate.year != 3000) _selectedDueDate = potentialDate;
    }

    if (widget.initialData['subtasks'] != null) {
      _subtasks =
          List<Map<String, dynamic>>.from(widget.initialData['subtasks']);
    }
  }
      ...
      builder: (context, child) => Theme(
        data: ThemeData.dark().copyWith(
          colorScheme: ColorScheme.dark(
              primary: _gold, onSurface: Colors.white, surface: _cardBg),
        ...
        if (combined.isBefore(DateTime.now())) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: const Text('La fecha no puede ser en el pasado',
  ...
  Future<void> _updateTask() async {
    ...
    try {
      await _notificationService.cancelTaskNotifications(widget.taskId);
      ...
      if (_selectedDueDate != null &&
          _selectedDueDate!.isAfter(DateTime.now())) {
  ...
  void _addSubtask() {
    ...
    if (t.isNotEmpty) {
      setState(() {
        _subtasks.add({'title': t, 'completed': false});
                  ...
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color:
                            isSelected ? col.withValues(alpha: 0.15) : _cardBg,
                        borderRadius: BorderRadius.circular(20),
                        border: Border.all(
                      ...
                      child: Text(
                        p.toUpperCase(),
                        style: TextStyle(
                            color: isSelected ? col : Colors.white54,
                            fontWeight: FontWeight.bold,
                            fontSize: 13),
                      ),
                    ),
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      leading: GestureDetector(
                        onTap: () => setState(
                            () => _subtasks[index]['completed'] = !done),
                        child: Container(
                          width: 22,
                          height: 22,
                          decoration: BoxDecoration(
                        ...
                        onPressed: () async {
                          final confirm = await showDialog<bool>(
                            ...
                            builder: (_) => AlertDialog(
                              backgroundColor: _cardBg,
                              title: const Text('¿Eliminar sub-tarea?',
                                  style: TextStyle(color: Colors.white)),
                              content: Text('Se borrará "${sub['title']}".',
                                  style:
              ...
              children: [
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                        color: _gold, borderRadius: BorderRadius.circular(16)),
